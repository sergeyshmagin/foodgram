...
from foodgram.constants import (
    MAX_RECIPE_NAME_LENGTH, MIN_COOKING_TIME, MAX_COOKING_TIME
)
...
class TestRecipeModel:
    ...
    def test_recipe_cooking_time_validation(self, user, cooking_time, should_raise):
    ...
    def test_favorite_unique_together(self, user, recipe):
        ...
        with pytest.raises(IntegrityError):
            Favorite.objects.create(
                user=user,
                recipe=recipe  # Дублирующаяся связь
            )
    ...
    def test_subscription_unique_together(self, user, another_user):
        ...
        with pytest.raises(IntegrityError):
            Subscription.objects.create(
                user=user,
                author=another_user  # Дублирующаяся связь
            ) 