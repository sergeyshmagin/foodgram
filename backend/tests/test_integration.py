...
class TestRecipeManagementFlow:
    ...
    def test_recipe_lifecycle(self, authenticated_client, tag, ingredient):
        ...
        recipe_data = {
            ...
            'tags': [tag.id],
            'ingredients': [
                {'id': ingredient.id, 'amount': 250}
            ]
        }
        response = authenticated_client.post('/api/v1/recipes/', recipe_data, format='json')
        assert response.status_code == status.HTTP_201_CREATED
        recipe_id = response.data['id']
        
        # 2. Добавление в избранное
        response = authenticated_client.post(f'/api/v1/recipes/{recipe_id}/favorite/')
        assert response.status_code == status.HTTP_201_CREATED
        ...
        response = authenticated_client.post(f'/api/v1/recipes/{recipe_id}/shopping_cart/')
        assert response.status_code == status.HTTP_201_CREATED
...
class TestSubscriptionFlow:
    ...
    def test_subscription_flow(self, authenticated_client, another_user, tag, ingredient):
        ...
        response = api_client.post('/api/v1/recipes/', recipe_data, format='json')
        assert response.status_code == status.HTTP_201_CREATED
        
        # 2. Подписываемся на автора
        response = authenticated_client.post(f'/api/v1/users/{another_user.id}/subscribe/')
        assert response.status_code == status.HTTP_201_CREATED
...
class TestFilteringFlow:
    ...
    def test_complex_filtering(self, authenticated_client, another_user):
        ...
        from apps.recipes.models import Tag, Ingredient
        
        # Создаем тег и ингредиент
        tag = Tag.objects.create(name='Завтрак', slug='breakfast')
        ingredient = Ingredient.objects.create(name='Яйца', measurement_unit='шт')
        ...
        # Фильтрация по тегу
        response = authenticated_client.get(f'/api/v1/recipes/?tags={tag.slug}')
        assert response.status_code == status.HTTP_200_OK
        assert response.data['count'] == 1
        
        # Фильтрация по автору
        response = authenticated_client.get(f'/api/v1/recipes/?author={another_user.id}')
        assert response.status_code == status.HTTP_200_OK
        assert response.data['count'] == 1
...
class TestErrorHandlingFlow:
    ...
    def test_error_scenarios(self, authenticated_client, recipe):
        ...
        response = authenticated_client.post(f'/api/v1/recipes/{recipe.id}/favorite/')
        assert response.status_code == status.HTTP_201_CREATED
