...
class TestUserViewSetEdgeCases:
    ...
    def test_set_password_invalid_current_password(self, authenticated_client):
        ...
        response = authenticated_client.post('/api/v1/users/set_password/', data)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    
    def test_set_avatar_invalid_format(self, authenticated_client):
        """Тест установки аватара в неверном формате."""
        data = {
            'avatar': 'invalid_base64_string'
        }
        response = authenticated_client.put('/api/v1/users/me/avatar/', data, format='json')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestSubscriptionViewSetEdgeCases:
    ...
    def test_subscribe_to_self(self, authenticated_client, user):
        ...
        response = authenticated_client.post(f'/api/v1/users/{user.id}/subscribe/')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    
    def test_subscribe_twice(self, authenticated_client, another_user):
        """Тест повторной подписки на того же пользователя."""
        # Первая подписка
        response1 = authenticated_client.post(f'/api/v1/users/{another_user.id}/subscribe/')
        assert response1.status_code == status.HTTP_201_CREATED
        
        # Повторная подписка
        response2 = authenticated_client.post(f'/api/v1/users/{another_user.id}/subscribe/')
        assert response2.status_code == status.HTTP_400_BAD_REQUEST
    
    def test_unsubscribe_when_not_subscribed(self, authenticated_client, another_user):
        """Тест отписки когда не подписан."""
        response = authenticated_client.delete(f'/api/v1/users/{another_user.id}/subscribe/')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestRecipeViewSetEdgeCases:
    ...
    def test_create_recipe_without_image(self, authenticated_client, tag, ingredient):
        ...
        response = authenticated_client.post('/api/v1/recipes/', data, format='json')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    
    def test_update_recipe_by_non_author(self, api_client, another_user, recipe):
        """Тест обновления рецепта не автором."""
        api_client.force_authenticate(user=another_user)
        data = {
            'name': 'Измененное название'
        }
        response = api_client.patch(f'/api/v1/recipes/{recipe.id}/', data, format='json')
...
class TestFavoriteViewSetEdgeCases:
    ...
    def test_add_to_favorites_twice(self, authenticated_client, recipe):
        ...
        # Первое добавление
        response1 = authenticated_client.post(f'/api/v1/recipes/{recipe.id}/favorite/')
        assert response1.status_code == status.HTTP_201_CREATED
        
        # Повторное добавление
        response2 = authenticated_client.post(f'/api/v1/recipes/{recipe.id}/favorite/')
        assert response2.status_code == status.HTTP_400_BAD_REQUEST
    
    def test_remove_from_favorites_when_not_favorited(self, authenticated_client, recipe):
        """Тест удаления из избранного когда рецепт не в избранном."""
        response = authenticated_client.delete(f'/api/v1/recipes/{recipe.id}/favorite/')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestShoppingCartViewSetEdgeCases:
    ...
    def test_add_to_shopping_cart_twice(self, authenticated_client, recipe):
        ...
        # Первое добавление
        response1 = authenticated_client.post(f'/api/v1/recipes/{recipe.id}/shopping_cart/')
        assert response1.status_code == status.HTTP_201_CREATED
        
        # Повторное добавление
        response2 = authenticated_client.post(f'/api/v1/recipes/{recipe.id}/shopping_cart/')
        assert response2.status_code == status.HTTP_400_BAD_REQUEST
    
    def test_download_empty_shopping_cart(self, authenticated_client):
        """Тест скачивания пустой корзины покупок."""
        response = authenticated_client.get('/api/v1/recipes/download_shopping_cart/')
