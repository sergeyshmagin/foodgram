...
class TestRecipeAPI:
    ...
    def test_recipe_create_unauthenticated(self, api_client, recipes_url, recipe_data):
        """Тест создания рецепта неаутентифицированным пользователем."""
        response = api_client.post(recipes_url, recipe_data)
        
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    
    def test_recipe_create_authenticated(self, authenticated_client, recipes_url, recipe_data):
        """Тест создания рецепта аутентифицированным пользователем."""
        response = authenticated_client.post(recipes_url, recipe_data, format='json')
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (MIN_COOKING_TIME, status.HTTP_201_CREATED),  # минимальное валидное
            (60, status.HTTP_201_CREATED),         # нормальное значение
            (MAX_COOKING_TIME, status.HTTP_201_CREATED),  # максимальное валидное
            (MAX_COOKING_TIME + 1, status.HTTP_400_BAD_REQUEST),  # превышение максимума
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([{'id': 1, 'amount': 0}], status.HTTP_400_BAD_REQUEST),  # нулевое количество
            ([{'id': 1, 'amount': -1}], status.HTTP_400_BAD_REQUEST),  # отрицательное количество
            ([{'amount': 100}], status.HTTP_400_BAD_REQUEST),  # без ID
            ([{'id': 1, 'amount': 100}], status.HTTP_201_CREATED),  # валидные данные
        ]
    )
    def test_recipe_ingredients_validation(
        self, authenticated_client, recipes_url, recipe_data, ingredient,
        ingredients_data, expected_status
    ):
